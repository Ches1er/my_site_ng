...
export class CampaignContentUnitComponent implements OnInit {
  ...
  private ifExpired() {
    const now = Date.now();
/*    console.log(this.datePipe.transform(now, 'short'));
    console.log(this.datePipe.transform(this.campaign.expiration, 'short'));*/
    console.log(now);
    console.log(this.campaign.expiration - now);
    console.log(this.campaign.expiration < now);
